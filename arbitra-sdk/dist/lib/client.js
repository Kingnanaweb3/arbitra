...
import { ArbitraErrorMessages, } from "../types/index.js";
// ── Constants ─────────────────────────────────────────────
...
export class ArbitraClient {
    ...
    async createPolicy(config) {
        ...
        const policyObject = createdObjects.find((obj) => obj.type === "created" && "objectType" in obj &&
    ...
    async validateAction(policyId, request) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
    ...
    // ── Revoke policy ───────────────────────────────────────
    async revokePolicy(policyId, capId) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    // ── Pause policy ────────────────────────────────────────
    async pausePolicy(policyId, riskScore) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    // ── Resume policy ───────────────────────────────────────
    async resumePolicy(policyId, riskScore) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    // ── Update risk params ──────────────────────────────────
    async updateRiskParams(policyId, newRiskCeiling, newSlippageBps, newMaxTx) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    async getPolicyEvents(policyId) {
    ...
    async activateStressTest(riskParamsId, injectedScore) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    async deactivateStressTest(riskParamsId) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
    ...
    async updateRiskScore(riskParamsId, volatilityScore, oracleScore, depthScore) {
        if (!this.keypair)
            throw new Error("No keypair configured for signing");
        const tx = new Transaction();
        tx.moveCall({
        ...
        const aggregate = Math.floor((volatilityScore * 40 + oracleScore * 30 + depthScore * 30) / 100);
        return {
            aggregate,
            volatilityScore,
            oracleScore,
